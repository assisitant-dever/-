...
export default function Sidebar({ onSelect }: { onSelect: (c: any) => void }) {
  ...
  const newConv = async () => {
    try {
      ...
      // 3️⃣ 之后在 CorePage 的 useEffect 中监听 messages，
      // 如果是第一条 AI 回复，就调用生成标题接口：
      // api.generateTitle(newConversation.id) → dispatch 更新 title
    } catch (err) {
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="self-end ml-auto top-1 z-50 w-6 h-6 flex items-center justify-center bg-slate-100 dark:bg-slate-900 text-slate-800 dark:text-slate-200 rounded-full transition-all duration-300 hover:scale-110 hover:shadow-md"
            title="收回侧边栏"
        ...
        <input
          ...
          className={`mb-3 mt-3 px-3 py-1 text-sm border rounded bg-white dark:bg-slate-800 dark:border-slate-600 ${isOpen ? '' : 'invisible'}`}
        />

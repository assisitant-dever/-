...
from .models import AIModelResponse,PlatformModelResponse,User
load_dotenv()
...
async def create_ai_model(
    ...
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)  # 确保用户身份
):
    # 创建 AI 模型实例
    new_ai_model = AIModel(
        platform_name=ai_model_create.platform,
        model_name=ai_model_create.model_name,
        api_key=ai_model_create.api_key,  # 这里暂时存储用户提供的明文 API Key
        user_id=current_user.id,
    ...
    return AIModelResponse(
        ...
        api_key_mask=new_ai_model.api_key[:5] + "****",  # 仅返回部分 API Key

...
async def generate_conversation_title(
    ...
    content = " ".join([m.content for m in messages if m.content])  # 避免 None
    if not content:
        raise HTTPException(status_code=400, detail="No content available for title generation")
    
    print("生成标题用内容:", content)

    try:
        # 确保 prompt 是字符串，可序列化
        title = await generate_text_for_user(
            user_id=conversation.user_id,
            prompt=str(content),  # ✅ 强制转换为字符串
            db=db
        )
    except Exception as e:
        print("AI 生成标题出错:", e)
    ...
    conversation.title = title.strip() if title else "未命名会话"
    db.commit()

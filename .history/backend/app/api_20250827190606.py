...
async def update_ai_model(
    ...
    if not ai_model_update.model_id:
        raise HTTPException(status_code=400, detail="model_id不能为空")
...
async def generate_conversation_title(conversation_id: int, db: Session = Depends(get_db)):
    ...
    # 调用大模型生成简短标题
    prompt = f"请为以下对话生成一个简短的标题（不超过15字）：\n\n{content}"
    title = await generate_text_for_user(prompt)  # 你已有的 AI 调用方法

...
async def generate_conversation_title(conversation_id: int, db: Session = Depends(get_db)):
    ...
    prompt = " ".join([m.content for m in messages if m.content]) or "无消息内容"
    print("生成标题用内容:", prompt)

    try:
        # 流式生成，只获取第一条 AI 输出作为标题
        ai_stream = generate_text_for_user(
        ...
        first_chunk = ""
        for chunk in ai_stream:  # chunk 是迭代器每次返回的内容
            first_chunk += chunk
            break  # 只取第一块
        title = first_chunk.strip() or "未生成标题"
    except Exception as e:
        print("AI 生成标题出错:", e)

...
def get_user_api_keys(
    ...
    for item in user_api_keys:
        # 示例：保留前4位，其余用*代替（根据实际需求调整）
        if item.api_key:  # 假设原始key存储在api_key字段
            item.api_key_mask = f"{item.api_key[:4]}****{item.api_key[-4:]}"
...
async def create_ai_model(
    ...
    new_ai_model = AIModel(
        ...
        api_key=ai_model_create.api_key,
        user_id=current_user.id,
...
async def update_ai_model(
    ...
    ai_model.api_key = ai_model_create.api_key
    ai_model.updated_at = datetime.now(pytz.UTC)  # 更新时间

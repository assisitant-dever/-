...
class QwenClient(BaseAIClient):
    ...
    def stream_generate(self, prompt: str, system_prompt: str = "You are a helpful assistant.") -> Iterator[str]:
        ...
        stream = self.client.chat.completions.create(
            ...
            stream=True,  # 开启流式
            stream_options={"include_usage": True}  # 可选：获取Token使用统计
        )
        
        for chunk_idx, chunk in enumerate(stream):
            # 1. 过滤空choices（防止无有效片段）
            if not chunk.choices or len(chunk.choices) == 0:
                continue
            print(f"[Qwen 流式片段 {chunk_idx}] 存在choices: {bool(chunk.choices)}")

            choice = chunk.choices[0]
            # 2. 检测流式结束标记（如finish_reason为"stop"，停止迭代）
            if choice.finish_reason is not None:
                print(f"[Qwen 流式结束] finish_reason: {choice.finish_reason}")
                break
            
            # 3. 安全处理ChoiceDelta：直接访问content属性，判断非空
            # ChoiceDelta的content属性可能为None，需显式判断
            if choice.delta and choice.delta.content is not None:
                print(f"[Qwen 有效片段 {chunk_idx}] 内容: {content}")  # 打印有效片段
                content = choice.delta.content.strip()  # 过滤空字符串
                if content:
                    yield content

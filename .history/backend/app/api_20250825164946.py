...
from .models import Conversation, Message  # 导入会话/消息模型
load_dotenv()
...
async def generate_document(
    doc_type: str = Form(...),
    user_input: str = Form(...),
    conv_id: int = Form(None),  # 允许新会话（conv_id为None时创建）
    ai_model_id: int = Form(None),  # 可选：用户手动选择的模型ID
    ...
    # 步骤1：获取用户模型配置（优先手动选择的ai_model_id，其次默认偏好）
    user_config = None
    ...
    if ai_model_id:
        ...
        # 格式化用户配置（适配generate_text_for_user入参）
        user_config = {
        ...
        if not user_config:
            raise HTTPException(status_code=400, detail="请先添加AI模型配置（API Key）")
        # 同步获取selected_ai_model（用于后续关联Conversation）
        selected_ai_model = db.query(AIModel).filter(
    ...
    # 步骤2：调用多平台客户端生成内容（传入用户配置）
    try:
        generated = generate_text_for_user(
            user_id=current_user.id,
            prompt=prompt,
            db=db,  # 传入db会话
            system_prompt=base_prompt  # 传入公文类型的专属system prompt（更精准）
        )
...
async def delete_conversation(
    ...
    # 2. 删除关联的Message（级联删除，也可在模型定义时配置ondelete="CASCADE"）
    db.query(Message).filter(Message.conversation_id == conv_id).delete()
    # 3. 删除Conversation
    db.delete(conversation)
    db.commit()

    return JSONResponse({"message": "会话删除成功"})

...
from sqlalchemy.orm import Session, joinedload,Select
...
async def get_user_ai_configs(
    ...
    db: AsyncSession = Depends(get_async_db)  # 假设 get_async_db 返回 AsyncSession
):
    # 异步查询：使用 select + options + scalars().all()
    ...
    user_configs = result.scalars().all()  # 获取结果
    ...
    for config in user_configs:
        ...
        if len(decrypted_key) <= 8:
            config.api_key_mask = f"{decrypted_key[:4]}***" if decrypted_key else "***"
        else:
            config.api_key_mask = f"{decrypted_key[:4]}***{decrypted_key[-4:]}" if decrypted_key else "***"

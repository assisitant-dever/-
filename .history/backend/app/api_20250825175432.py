...
# ----------------- 接口：新增AI模型配置（加密存储） -----------------
@router.post("/api/keys", response_model=AIModelResponse)
async def create_ai_model(
    # 关键修改：请求模型从 AIModelCreate（原）改为新的 AIModelCreate（用 model_id 关联系统模型）
    ai_model_create: AIModelCreate,  
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user)
):
    """
    修改：用户新增AI模型配置（基于系统模型ID，不再手动输入平台/模型名称）
    """
    # 1. 校验系统模型是否存在且支持
    ...
    if existing_config:
        raise HTTPException(status_code=400, detail=f"您已为「{system_model.platform.name}-{system_model.name}」配置过API Key，无需重复添加")
...
@router.get("/api/keys", response_model=List[AIModelResponse])
async def get_user_ai_configs(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """
    修改：获取用户所有AI配置（自动关联系统平台/模型名称，避免冗余存储）
    """
    user_configs = db.query(AIModel).filter(
        AIModel.user_id == current_user.id
    ).order_by(AIModel.created_at.desc()).all()
    
    # Pydantic 模型会自动通过 @field_validator 填充 platform_name/model_name
    return user_configs
# ----------------- 接口：删除AI模型配置 -----------------

...
async def get_user_ai_configs(
    ...
    # 关键：预加载 model 和 platform 关联（级联加载）
    user_configs = db.query(AIModel).filter(
        AIModel.user_id == current_user.id
    ).options(
        # 加载 AIModel → Model 关联
        joinedload(AIModel.model),
        # 加载 Model → Platform 关联（因为 AIModel.platform 依赖 Model.platform）
        joinedload(AIModel.model).joinedload(Model.platform)
    ).order_by(AIModel.created_at.desc()).all()
    
    # 其他逻辑（如 API Key 脱敏）保持不变
    for config in user_configs:
        # 注意：这里需要使用解密后的 API Key 进行脱敏
        decrypted_key = decrypt_api_key(config.api_key)  # 假设你有解密函数

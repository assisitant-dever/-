...
async def generate_title(conversation_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # 1. 获取会话
    conversation = db.query(Conversation).filter(
    ...
    for m in messages:
        ...
        if m.content:
            if isinstance(m.content, str):
                ...
                content = str(m.content.get("text", ""))
            elif isinstance(m.content, list):
                content = " ".join(str(item.get("text", "")) if isinstance(item, dict) else str(item) for item in m.content)
    ...
    logging.info(f"生成标题用内容: {prompt}")

    # 5. 调用 AI 生成标题
    try:
        title = ai_client.generate_title(prompt)  # 替换成你的 AI 调用方法
    except Exception as e:
        logging.error(f"AI 生成标题出错: {e}")

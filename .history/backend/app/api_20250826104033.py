...
async def generate_document(
    ...
    if ai_model_id:
        ...
        # 组装用户配置（从关联表获取平台/模型信息）
        user_config = {
            "provider": selected_ai_model.model.platform.name,  # 从Model→Platform获取平台名
...
async def get_user_ai_configs(
    ...
    try:
        ...
        stmt = select(AIModel).filter(
            ...
            # 预加载AIModel→Model→Platform完整关联
            joinedload(AIModel.model).joinedload(Model.platform)
        ).order_by(AIModel.created_at.desc())
        
        # 2. 执行异步查询并获取结果
        result = await db.execute(stmt)
        user_configs = result.scalars().unique().all()  # 使用unique()确保关联数据不重复
        # 2. 强制打印：检查每个 AIModel 的 model 是否存在
        print("\n===== 开始检查 AIModel 关联 =====")
        for config in user_configs:
            print(f"AIModel ID: {config.id}")
            print(f"关联的 model_id: {config.model_id}")  # 打印数据库中存储的 model_id
            print(f"model 属性是否存在: {config.model is not None}")  # 核心检查
            if config.model:
                print(f"model 的 platform_id: {config.model.platform_id}")
                print(f"model.platform 是否存在: {config.model.platform is not None}")
            else:
                print("❌ model 属性为 None（未关联任何模型）")
        print("===== 结束检查 AIModel 关联 =====\n")
        # 3. 验证关联数据是否有效（关键补充）
        for config in user_configs:
            # 检查Model关联
            if not config.model:
                raise HTTPException(
                    status_code=500,
                    detail=f"AI配置ID={config.id}未关联模型，请检查数据"
                )
            # 检查Platform关联
            if not config.model.platform:
                raise HTTPException(
                    status_code=500,
                    detail=f"模型ID={config.model.id}未关联平台，请检查数据"
                )
        ...
        for config in user_configs:
            try:
                ...
                print(f"解密API Key失败（配置ID={config.id}）：{str(e)}")
                decrypted_key = ""
            
            # 脱敏逻辑
            if len(decrypted_key) <= 8 and decrypted_key:
                config.api_key_mask = f"{decrypted_key[:4]}***"
            elif decrypted_key:
                config.api_key_mask = f"{decrypted_key[:4]}***{decrypted_key[-4:]}"
            else:
                config.api_key_mask = "***"  # 完全脱敏
        
        # 5. 手动验证响应模型（调试用，生产可移除）
        try:
            validated_response = [AIModelResponse.model_validate(config) for config in user_configs]
        except ValidationError as e:
            print(f"响应模型验证失败：{e}")
            raise HTTPException(status_code=500, detail="数据格式错误")
        except Exception as e:
        # 关键：打印完整异常栈（包含错误位置和原因）
            print("="*50)
            print("接口执行异常详细信息：")
            print(traceback.format_exc())  # 打印完整错误栈
            print("="*50)

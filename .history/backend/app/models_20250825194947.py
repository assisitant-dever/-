...
class AIModelResponse(BaseModel):
    ...
    def get_platform_name(cls, v, info: ValidationInfo):
        # info.data 是 SQLAlchemy 的 AIModel 实例
        aimodel_instance = info.data
        # 从 AIModel 的 platform 属性（@property 定义）获取关联的 Platform 实例
        if hasattr(aimodel_instance, "platform") and aimodel_instance.platform:
            return aimodel_instance.platform.name
        # 若关联缺失，抛出明确错误（提示预加载关联）
        raise ValueError(f"AIModel(id={aimodel_instance.id}) 未关联平台信息，请检查查询时是否预加载关联")

    @field_validator("model_name", mode="before")
    def get_model_name(cls, v, info: ValidationInfo):
        aimodel_instance = info.data
        # 从 AIModel 的 model 属性（SQLAlchemy 关系）获取关联的 Model 实例
        if hasattr(aimodel_instance, "model") and aimodel_instance.model:
            return aimodel_instance.model.name
        raise ValueError(f"AIModel(id={aimodel_instance.id}) 未关联模型信息，请检查查询时是否预加载关联")

    # API Key 脱敏（保留前4后4，中间用***替代）
    @field_validator("api_key_mask", mode="before")
    def mask_api_key(cls, v, info: ValidationInfo):
        aimodel_data = info.data  # 可能是字典（前端提交）或 SQLAlchemy 实例（后端查询）
        
        # 兼容逻辑：先判断是否为字典，再判断是否为 SQLAlchemy 实例
        if isinstance(aimodel_data, dict):
            # 场景1：前端提交数据（如新增/编辑），info.data 是字典
            raw_api_key = aimodel_data.get("api_key", "")
        else:
            # 场景2：后端查询数据（如获取用户配置），info.data 是 SQLAlchemy 实例

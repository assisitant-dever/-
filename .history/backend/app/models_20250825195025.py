...
class AIModelResponse(BaseModel):
    ...
    def get_model_name(cls, v, info: ValidationInfo):
        ...
        if isinstance(aimodel_data, dict):
            # 场景1：前端提交时，从字典中直接获取（若前端传递了 model_name）
            return aimodel_data.get("model_name", "")
        else:
            # 场景2：后端查询时，从 SQLAlchemy 实例的关联属性获取
            if hasattr(aimodel_data, "model") and aimodel_data.model:
                return aimodel_data.model.name
            raise ValueError(f"AIModel(id={aimodel_data.id}) 未关联模型信息，请检查查询时是否预加载关联")

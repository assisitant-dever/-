    ...
    try:
        ...
        for chunk in generated_iterator:
            if chunk.strip():
                ...
                await asyncio.sleep(0.001)  # 强制推送，禁用缓冲

        # 2. 确保full_content不为空（即使AI返回短内容）
        generated_full = "".join(full_content).strip()
        if not generated_full:
            yield f"event: error\ndata: {json.dumps({'detail': 'AI生成内容为空'})}\n\n"
            return

        # -------------------------- 3. 强制推送最终事件 --------------------------
        # 3.1 生成DOCX和数据库存储（原有逻辑不变）
        # ...（此处省略DOCX生成、document_history/messages插入代码）...

        # 3.2 推送metadata事件（必带full_text）
        metadata = {
            "filename": filename,
            "conv_id": conversation.id,
            "doc_id": doc_record.id,
            "used_model": f"{selected_ai_model.model.platform.name} - {selected_ai_model.model.name}",
            "full_text": generated_full,  # 确保携带完整内容
        }
        yield f"event: metadata\ndata: {json.dumps(metadata)}\n\n"
        await asyncio.sleep(0.001)  # 强制推送

        # 3.3 推送complete事件（标记结束）
        yield f"event: complete\ndata: {json.dumps({'status': 'success', 'msg': '生成完成'})}\n\n"
        await asyncio.sleep(0.001)

    except Exception as e:
        # 异常时也推送error事件，避免前端一直等待
        error_detail = str(e)
        # ...（原有错误格式化逻辑）...

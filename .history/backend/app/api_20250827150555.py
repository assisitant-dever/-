...
async def get_conversation_detail(
    ...
    conversation = db.query(Conversation).filter(
        ...
        joinedload(Conversation.messages).order_by(Message.created_at.asc()),
...
async def get_document_detail(
    ...
    doc = db.query(DocumentHistory).filter(
        ...
        joinedload(DocumentHistory.template)
    ).first()
    
    if not doc:
        raise HTTPException(status_code=404, detail="历史公文不存在或无权访问")
    ...
    return JSONResponse({
        ...
        "filename": doc.filename,
        "used_template": {
            "id": doc.template.id if doc.template else None,
            "original_name": doc.template.original_name if doc.template else None,
            "preview": doc.template.content[:50] + "..." if (doc.template and doc.template.content) else None
        } if doc.template else None,
        "created_at": doc.created_at.astimezone(pytz.timezone("Asia/Shanghai")).strftime("%Y-%m-%d %H:%M:%S"),
        "content": doc.content,
        "download_url": f"/api/download/{urllib.parse.quote(doc.filename)}"
    })
...
async def update_template(
    template_id: int,
    original_name: str = Form(..., description="模板显示名称"),
    content: str = Form(..., description="模板完整内容"),
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    template = db.query(Template).filter(
    ...
    if not template:
        raise HTTPException(status_code=404, detail="模板不存在或无权编辑")

    if not original_name.strip():
        raise HTTPException(status_code=400, detail="模板显示名称不能为空")
    if not content.strip():
        raise HTTPException(status_code=400, detail="模板内容不能为空")

    try:
        template.original_name = original_name.strip()
        template.content = content.strip()
        db.commit()
        db.refresh(template)
        
        return JSONResponse({
            "message": f"模板「{original_name}」编辑成功",
            "template": {
                "id": template.id,
                "original_name": template.original_name,
                "content_preview": template.content[:50] + "...",
                "updated_at": datetime.now(pytz.UTC).astimezone(pytz.timezone("Asia/Shanghai")).strftime("%Y-%m-%d %H:%M:%S")
            }
        })
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"编辑模板失败：{str(e)}")

...
async def generate_conversation_title(conversation_id: int, db: Session = Depends(get_db)):
    ...
    # 2️⃣ 获取前两条消息内容，组合成 prompt 字符串
    messages = db.query(Message).filter(Message.conversation_id == conversation_id).order_by(Message.id).limit(2).all()
    prompt = " ".join([m.content for m in messages if m.content]).strip()
    if not prompt:
        prompt = "无消息内容"

    print("生成标题用内容:", prompt)

    # 3️⃣ 调用 AI 流式生成，取第一条输出作为标题
    try:
        ai_stream: Iterator[str] = generate_text_for_user(
            user_id=conversation.user_id,
            prompt=prompt,
            db=db,
            stream=True
        )
        first_chunk = next(ai_stream, None)  # 获取第一条 AI 输出
        if not first_chunk:
            first_chunk = "未生成标题"

        # 4️⃣ 保存标题到数据库
        conversation.title = first_chunk.strip()
        db.commit()

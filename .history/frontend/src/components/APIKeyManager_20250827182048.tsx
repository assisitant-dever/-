...
const ApiKeyManager: React.FC<ApiKeyManagerProps> = ({ onConfigChange }) => {
  ...
  const handleEdit = () => {
    ...
    const matchedModel = systemModels.find(
      m => m.platform_name === currentConfig.platform_name && m.name === currentConfig.model_name
    );
    setFormState({
      model_id: matchedModel?.model_id || 0,
      api_key: '',
  ...
  const handleSaveOrAdd = async () => {
    const platformNameToSubmit = formState.custom_platform_name?.trim() || formState.platform_name;
    const modelNameToSubmit = formState.custom_model_name?.trim() || formState.model_name;

    if (!platformNameToSubmit) return toast.warning("请输入平台名称");
    if (!modelNameToSubmit) return toast.warning("请输入模型名称");
    if (!formState.api_key.trim()) return toast.warning("请输入API Key");
    ...
    try {
      if (currentConfig) {
        ...
        const res = await api.put<ApiKeyConfig>(`/api/keys/${currentConfig.id}`, {
          ...
          platform_name: platformNameToSubmit,
          model_name: modelNameToSubmit
        });
        setApiKeyConfigs(apiKeyConfigs.map(c => c.id === currentConfig.id ? res.data : c));
        ...
        setApiKeyConfigs([...apiKeyConfigs, res.data]);
        setCurrentConfig(res.data);
        toast.success("新增成功");
      }
      setIsEditing(false);
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          {!loading && currentConfig ? (
            <div className="px-4 py-2 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center gap-2">
              ...
              <button onClick={handleEdit} className="ml-2 px-2 py-1 bg-blue-600 text-white text-xs rounded hover:bg-blue-700 transition-colors">
                编辑
              </button>
            </div>
          ) : (
          ...
          <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-green-600 text-white text-sm rounded hover:bg-green-700 transition-colors" disabled={loading}>
            新增配置
          </button>
        </div>
      </div>
      ...
      {isEditing && (
        <div className="p-5 bg-slate-50 dark:bg-slate-700/50 rounded-lg space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleSaveOrAdd} className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
              {loading ? '处理中...' : currentConfig ? '保存修改' : '新增配置'}
            </button>
          </div>
        </div>
      )}
        ...
        {loading ? (
          ...
          <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead className="bg-slate-50 dark:bg-slate-800">
              <tr>
                <th>平台</th><th>模型名称</th><th>API Key</th><th>BaseURL</th><th>创建时间</th><th>状态</th><th>操作</th>
              </tr>
            </thead>
            <tbody>
              {apiKeyConfigs.map(config => (
                <tr key={config.id}>
                  ...
                  <td>{config.api_key_mask}</td>
                  <td>{config.base_url || '默认'}</td>
                  <td>{new Date(config.created_at).toLocaleString()}</td>
                  <td>{currentConfig?.id === config.id && <span>正在使用</span>}</td>
                  <td>
                    <button onClick={() => handleSelectConfig(config)}>选择</button>
                    <button onClick={() => handleDelete(config.id, config.platform_name, config.model_name)}>删除</button>
                  </td>
                </tr>
              ))}
            </tbody>

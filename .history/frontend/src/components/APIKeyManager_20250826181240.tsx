...
useEffect(() => {
  // 组件挂载时调用initData初始化数据
  initData();
}, []); // 空依赖数组表示只在组件首次渲染后执行一次
  // 修改initData函数中的数据处理
const initData = async () => {
  ...
  setApiKeyConfigs(configsRes.data); // 新增这行

    // 关键：打印后端返回的原始平台数据，确认是否有值
    console.log("后端返回的原始platforms数据：", platformsRes.data);
    
    // 过滤已启用的平台
    const activePlatforms = platformsRes.data.filter(platform => platform.is_active);
    setPlatformList(activePlatforms);
    
    // 关键：打印过滤后的平台列表，确认是否有5个平台
    console.log("过滤后的活跃平台：", activePlatforms);
...
// 修改fetchModelsByPlatform函数，适配新的响应结构
const fetchModelsByPlatform = async (platformId: number) => {
  try {
    // 1. 先从已加载的 platformList 中获取（减少后端请求，优化性能）
    const platformFromList = platformList.find(p => p.platform_id === platformId);
    if (platformFromList && platformFromList.model_details) {
      // 直接返回平台列表中已有的 model_details（过滤支持的模型）
      return platformFromList.model_details.filter(model => model.is_supported);
    }

    // 2. 若平台列表中无数据，再请求后端接口（传 include_details=true）
    const res = await api.get<PlatformModelResponse>(
      `/api/platforms/${platformId}?include_details=true`
    );

    // 3. 校验平台是否存在且启用
    if (!res.data.is_active) {
      toast.warning("该平台已禁用，无法获取模型");
      return [];
    }

    // 4. 返回真实的 model_details 数据（过滤支持的模型）
    return res.data.model_details?.filter(model => model.is_supported) || [];
  } catch (error) {
    console.error(`获取平台${platformId}模型失败:`, error);
    toast.error("加载模型列表失败：服务器异常或平台不存在");
    return [];
  }
};
              ...
              <label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">
                ...
                <select
                  ...
                  onChange={async (e) => {
                    ...
                    // 从后端返回的 platformList 找到选中的平台
                    const selectedPlatform = platformList.find(p => p.platform_id === platformId);
                    if (selectedPlatform) {
                      // 从 platformList 提取模型，或调用函数请求后端
                ...
                >
                  ...
                  {/* 从 platformList 渲染所有已启用的平台 */}
                  {platformList.map(platform => (
                    <option key={platform.platform_id} value={platform.platform_id}>
                      {platform.platform} {/* 显示真实平台名称，如 "Alibaba"、"OpenAI" */}
                    </option>
                  ))}
                </select>
            </div>

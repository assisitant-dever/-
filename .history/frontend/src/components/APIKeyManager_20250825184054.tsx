...
const initData = async () => {
  ...
  try {
    ...
    const [configsRes, platformsRes] = await Promise.all([
      ...
      api.get<PlatformModelResponse[]>('/api/platforms')  // 后端返回的平台列表
    ]);
    
    // 2. 处理平台和模型数据，提取所有支持的模型
    const allModels: ModelDetail[] = [];
    platformsRes.data.forEach(platform => {
      // 从platform.model_details中收集所有支持的模型
      if (platform.model_details && platform.is_active) {
        const supportedModels = platform.model_details
        ...
        allModels.push(...supportedModels);
      }
    });
    
    // 3. 存储处理后的数据
    setApiKeyConfigs(configsRes.data);
    setSystemModels(allModels);  // 现在systemModels是ModelDetail类型数组

...
const handleEdit = () => {
  ...
  console.log("systemModels:", systemModels);

  // 找到匹配的系统模型
  const matchedModel = systemModels.find(
    m =>
      m.platform_name?.trim().toLowerCase() === currentConfig.platform_name?.trim().toLowerCase() &&
      m.name?.trim().toLowerCase() === currentConfig.model_name?.trim().toLowerCase()
  );


  if (!matchedModel) return toast.warning("未找到对应模型，请先选择模型");

  setFormState({
    model_id: matchedModel.model_id,  // ⚠️ 必须有 model_id
    api_key: '',
  ...
  const handleSaveOrAdd = async () => {
    ...
    const payload: any = {
      ...
      platform_name: formState.custom_platform_name?.trim() || formState.platform_name,
      model_name: formState.custom_model_name?.trim() || formState.model_name
    };

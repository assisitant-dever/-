...
const initData = async () => {
  ...
  try {
    const [configsRes, platformsRes] = await Promise.all([
      ...
      // 关键：传 include_details=true，获取后端返回的真实 model_details
    ...
    console.log("后端返回的平台列表：", activePlatforms);

    // 3. 初始加载第一个平台的真实模型数据（从 model_details 提取）
    if (activePlatforms.length > 0 && activePlatforms[0].model_details) {
      // 直接使用后端返回的 model_details，无需临时生成

...
const ApiKeyManager: React.FC<ApiKeyManagerProps> = ({ onConfigChange }) => {
  ...
  // 1. 加载用户API配置 + 系统支持的模型列表
  const initData = async () => {
    setLoading(true);
    try {
      // 并行请求：提高加载效率
      const [configsRes, modelsRes] = await Promise.all([
        api.get<ApiKeyConfig[]>('/api/keys'), // 用户已配置的API列表
        api.get<SystemModel[]>('/api/platforms/models') // 系统支持的所有模型
      ]);
      ...
      // 存储系统模型（仅保留支持的模型）
      setSystemModels(modelsRes.data.filter(model => model.is_supported));
    } catch (error) {
  ...
  const fetchModelsByPlatform = async (platformId: number) => {
    try {
      const res = await api.get<SystemModel[]>(`/api/platforms/${platformId}/models`);

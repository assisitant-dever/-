...
interface ModelDetail {
  model_id: string;          // 模型唯一标识（如 "gpt-4-turbo"）
  name: string;              // 模型显示名称（如 "GPT-4 Turbo"）
  description?: string;      // 模型描述
  capabilities: string[];    // 模型能力（如 ["chat", "vision"]）
  context_window?: number;   // 上下文窗口大小（token数）
  max_tokens?: number;       // 最大输出token数
  supports_tools?: boolean;  // 是否支持函数调用/tool use
  supports_vision?: boolean; // 是否支持视觉输入
  is_multimodal?: boolean;   // 是否是多模态模型
  updated_at?: string;       // 最后更新时间（ISO格式字符串）

...
const ApiKeyManager: React.FC = () => {
  ...
  useEffect(() => {
    // 初始化时从本地存储或 API 获取配置
    const savedConfigs = JSON.parse(localStorage.getItem('apiKeyConfigs') || '[]');
    setApiKeyConfigs(savedConfigs);
  ...
  const handleSave = () => {
    ...
    localStorage.setItem('apiKeyConfigs', JSON.stringify(updatedConfigs));
  ...
  const handleAddNew = () => {
    ...
    const updatedConfigs = [...apiKeyConfigs, { ...newConfig, id: newId }];
    setApiKeyConfigs(updatedConfigs);
    localStorage.setItem('apiKeyConfigs', JSON.stringify(updatedConfigs));
    setCurrentConfig({ ...newConfig, id: newId });
    setIsEditing(false);
  };

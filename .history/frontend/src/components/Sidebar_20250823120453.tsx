 ...
 return (
  <>
    ...
    <button
      ...
      className="absolute top-4 z-50 w-10 h-10 flex items-center justify-center
                 bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200
                 rounded-r-md hover:bg-slate-300 dark:hover:bg-slate-600
                 transition-transform duration-300 ease-in-out
                 transform-gpu"
      style={{
        left: isOpen ? '256px' : '40px', // ✅ 按钮始终贴在侧边栏右侧
      }}
    ...
    <div
      className={`absolute top-0 left-0 h-full bg-slate-100 dark:bg-slate-900 p-4 flex flex-col border-r
                  transform-gpu transition-all duration-300 ease-in-out z-40`}
      style={{
        width: isOpen ? '256px' : '40px', // ✅ 宽度变化，而不是 translate-x
      }}
    >
      {/* ========== 收起时只显示按钮 ========== */}
      {!isOpen && (

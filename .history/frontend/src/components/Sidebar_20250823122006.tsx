  ...
  return (
    <>
      ...
      <button
        ...
        className="absolute top-4 left-4 z-50 w-10 h-10 flex items-center justify-center bg-slate-200 dark:bg-slate-700 text-slate-800 dark:text-slate-200 rounded-r-md hover:bg-slate-300 dark:hover:bg-slate-600 transition-transform duration-300 ease-in-out transform-gpu"
        style={{
          transform: isOpen ? "translateX(256px)" : "translateX(0)", // 控制侧边栏动画效果
        }}
      ...
      {/* ========== 侧边栏主体（带动画）========== */}
      <div
        className={`absolute top-0 left-0 h-full bg-slate-100 dark:bg-slate-900 p-4 flex flex-col border-r transform-gpu transition-transform duration-300 ease-in-out z-40 ${
      ...
      >
        ...
        <Button className="mb-3" onClick={newConv}>
          ➕ 新建对话
        </Button>
              ...
              >
                {editingId === c.id ? (
                  ...
                  <div onClick={(e) => e.stopPropagation()} className="flex gap-1">
                    ...
                    />
                    <button onClick={saveEdit} className="text-green-600 text-xs px-1">
                      ✓
                    </button>
                  </div>
                ) : (

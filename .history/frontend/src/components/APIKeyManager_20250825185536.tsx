...
const initData = async () => {
    ...
    try {
      const [configsRes, platformsRes] = await Promise.all([
        ...
        // 关键：传include_details=true，让后端尝试返回model_details（即使目前是null）
      ...
      console.log("后端返回的平台列表：", activePlatforms); // 调试：确认有数据

      // 初始加载第一个平台的模型（从models字段临时提取，后续用model_details）
      if (activePlatforms.length > 0) {
        const firstPlatform = activePlatforms[0];
        // 临时：从models字段生成假的model数据（因为model_details是null）
        const tempModels = firstPlatform.models.map((modelName, index) => ({
          model_id: index + 1, // 临时用索引当ID（后续后端修复后替换为真实model_id）
          name: modelName,
          description: `平台 ${firstPlatform.platform} 的 ${modelName} 模型`,
          is_supported: true,
          platform_id: firstPlatform.platform_id,
          platform_name: firstPlatform.platform
        }));
        setSystemModels(tempModels);
      }
    } catch (error) {
...
const fetchModelsByPlatform = async (platformId: number) => {
  try {
    const res = await api.get<PlatformModelResponse>(
      `/api/platforms/${platformId}`  // 直接获取平台详情，包含模型
    );
    
    // 从平台详情中提取支持的模型
    if (res.data.model_details) {
      return res.data.model_details
        .filter(model => model.is_supported)
        .map(model => ({
          ...model,
          platform_id: res.data.platform_id,
          platform_name: res.data.platform
        }));
    }
    return [];
  } catch (error) {

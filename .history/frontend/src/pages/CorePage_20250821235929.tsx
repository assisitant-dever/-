...
const handleSend = async () => {
  ...
  try {
    ...
    const response = await api.post("/api/generate", {
      message: input,
      doc_type: docType,        // 后端字段命名习惯用下划线
      conversation_id: id,      // 当前会话 ID
    });

    const aiReply = response.data.reply;           // AI 回复内容
    const generatedDocument = response.data.doc;   // 生成的公文内容
    ...
    const errorMsg = err.response?.data?.detail || "AI 生成失败，请重试";
    setMessages((prev) => [
      ...prev,

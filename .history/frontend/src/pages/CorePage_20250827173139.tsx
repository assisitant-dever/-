        ...
        if (line.startsWith("data: ")) {
          ...
          if (errorLine) {
            try {
              ...
              // ✅ 新增：打印详细错误信息
              console.error("[流式错误-后端返回]", "errorLine:", errorLine, "errorData:", errorData);
              throw new Error(`后端流式错误: ${errorData.detail || "流式生成失败"}`);
            } catch (e) {
              console.error("[流式错误-解析errorLine失败]", "errorLine:", errorLine, "错误:", e);
              throw new Error(`解析错误响应失败: ${(e as Error).message}`);
            }
          } else {
            // ✅ 新增：处理无errorData的情况，不抛出异常（避免误判）
            console.warn("[流式警告-无错误数据]", "event: error但无data行", "lines:", lines);
            return; // 仅警告，不中断流式
          }
        }

        // 同时，在metadata处理逻辑中添加日志和防御：
          ...
          if (metaLine) {
            try {
              ...
              console.log("[流式 metadata]", "解析成功:", metadata);
            } catch (e) {
              // ✅ 仅打印警告，不抛出异常（metadata不是必需的，不影响核心流式）
              console.error("[流式警告-解析metadata失败]", "metaLine:", metaLine, "错误:", e);
            }
          } else {
            console.warn("[流式警告-无metadata数据]", "event: metadata但无data行", "lines:", lines);
          }
    }

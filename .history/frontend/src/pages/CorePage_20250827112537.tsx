...
export default function CorePage() {
  ...
  const [debugInfo, setDebugInfo] = useState<string>(""); // 用于显示调试信息
...
const handleWindowResize = () => {
  syncAllHeights();
  if (mainContainerRef.current) {
    const newMainWidth = mainContainerRef.current.offsetWidth;
    // 修复比例计算：当前中间栏宽度 / 原主容器宽度 = 保持比例
    const currentRatio = middleWidth / newMainWidth; 
    // 新宽度 = 新主容器宽度 * 原比例（确保拖拽后的比例不变）
    const newMiddleWidth = Math.floor(newMainWidth * currentRatio);
    // 重新应用宽度限制，避免超出范围
    const minWidth = 300;
    const maxWidth = newMainWidth - 200;
    setMiddleWidth(Math.max(minWidth, Math.min(newMiddleWidth, maxWidth)));
  }
};
// 初始化
useEffect(() => {
  ...
  const initTimeout = setTimeout(() => {
    setSidebarWidth(240);
    // 初始化中间栏宽度（仅首次渲染）
    if (mainContainerRef.current && middleWidth === 800) {
      const initWidth = Math.floor(mainContainerRef.current.offsetWidth * 0.5);
      setMiddleWidth(Math.max(300, Math.min(initWidth, mainContainerRef.current.offsetWidth - 200)));
    }
    syncAllHeights();
  }, 100); // 100ms延迟，确保Ref已获取到DOM

  // 窗口变化监听
  const handleWindowResize = () => { /* ... 同上 ... */ };
  window.addEventListener('resize', handleWindowResize);

  // 清理函数
  return () => {
    clearTimeout(initTimeout); // 清除延迟，避免内存泄漏
      ...
      >
        ...
        <div
          ...
          style={{ 
            ...
            borderRight: "1px solid #e2e8f0",
            transition: "width 0.05s ease-out", // 加快过渡，更跟手
            flexShrink: 0, // 关键：禁止flex压缩，确保宽度不被父容器强制调整
            flexGrow: 0, // 禁止flex拉伸，避免宽度被自动放大
            minWidth: "300px", // 与拖拽逻辑的minWidth对齐，双重保险
            maxWidth: "calc(100% - 200px)", // 与右侧栏minWidth呼应，避免溢出
          }}
        >
          ...
          <div
            ref={resizeHandleRef}
            style={{
              width: "10px", // 进一步扩大点击区域，更容易触发
              cursor: "col-resize",
              backgroundColor: isResizing ? "#2563eb" : "transparent", // 未拖拽时透明，不影响视觉
              opacity: isResizing ? 1 : 0.9,
              flexShrink: 0,
              flexGrow: 0,
              zIndex: 100, // 大幅提高层级，确保在所有元素上方（包括边框、内容）
              userSelect: "none", // 禁止拖拽时选中手柄，避免事件中断
              pointerEvents: "auto", // 强制启用鼠标事件，防止被父元素拦截
            }}

...
export default function CorePage() {
  ...
  const [middleWidth, setMiddleWidth] = useState<number | null>(null); // 中间栏宽度，初始为null自动计算
  ...
  const calculateInitialWidths = () => {
    ...
    // 中间栏初始占比50%，但不小于最小宽度
    const calculatedMiddleWidth = Math.max(400, containerWidth / 2);
    setMiddleWidth(calculatedMiddleWidth);
  };

  // 更新拖拽手柄位置
  const updateResizeHandlePosition = () => {
    if (!resizeHandleRef.current || !middleWidth) return;
    
    // 手柄位置 = 侧边栏宽度 + 中间栏宽度 - 手柄一半宽度
    const handleLeft = sidebarWidth + middleWidth - 2;
  ...
  const handleResizeMove = (e: MouseEvent) => {
    if (!isResizing || !mainContainerRef.current || middleWidth === null) return;
    
    const containerRect = mainContainerRef.current.getBoundingClientRect();
    const containerWidth = containerRect.width;
    
    // 计算新宽度 = 当前中间栏宽度 + (鼠标移动距离)
    const deltaX = e.clientX - startX;
    let newMiddleWidth = middleWidth + deltaX;
    
    // 限制宽度范围：最小400px，最大为主容器宽度-300px（确保右侧栏有足够空间）
    const minWidth = 400;
  ...
  useEffect(() => {
    ...
    const handleWindowResize = () => {
      // 窗口大小改变时重新计算宽度，但保持用户设置的比例
      if (mainContainerRef.current && middleWidth) {
        const containerWidth = mainContainerRef.current.offsetWidth;
        const currentRatio = middleWidth / (containerWidth + (middleWidth - (containerWidth / 2)));
        setMiddleWidth(Math.max(400, Math.min(containerWidth - 300, containerWidth * currentRatio)));
      }
      syncAllHeights();
      updateResizeHandlePosition();
    };
      ...
      >
        ...
        <div
          ...
          style={{ 
            ...
            width: middleWidth ? `${middleWidth}px` : '50%', 
            height: "100vh", 
            margin: 0,
            padding: 0,
            overflow: "hidden",

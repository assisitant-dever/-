  ...
  const addMessageToConversation = async (role: string, content: string, convId: number) => {
    ...
    try {
      const res = await api.post(`/api/conversations/${convId}/messages`, formData, {
        headers: {
          "Content-Type": "multipart/form-data",  // 可选，axios 会自动设置
        },
      });
...
const generateDocument = async () => {
  ...
  // formData.append("template_filename", "xxx.docx"); // 可选

  try {
    const res = await api.post("/api/generate", formData);
    
    // ✅ res.data 是后端返回的 { text, filename }
    setMessages((prev) => [
      ...prev,
      { 
        role: "assistant", 
        content: res.data.text,
        docx_file: res.data.filename  // 保存文件名，用于下载
      },
    ]);
  } catch (err: any) {
    console.error("生成失败", err.response?.data || err.message);
    alert("生成失败：" + (err.response?.data?.detail || "未知错误"));
  }
};

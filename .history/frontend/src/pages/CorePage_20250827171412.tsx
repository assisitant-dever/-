...
export default function CorePage() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let fullStreamingContent = ""; // 前端拼接完整流式内容
      // 1. 先添加空的AI消息占位符
        ...
        lines.forEach(line => {
          ...
          if (line.startsWith("data: ")) {
            try {
              ...
              if (data.chunk) {
                fullStreamingContent += data.chunk; // 前端拼接完整内容
                // 更新消息列表中的占位符内容（实时渲染）
                setMessages(prev => 
                  prev.map(msg => 
                    msg.id === tempMsgId ? { ...msg, content: fullStreamingContent } : msg
                  )
                );
                // 更新预览区（原有逻辑）
                setPreviewContent(fullStreamingContent);
              }
            } catch (e) {
          ...
          // 3. 处理metadata事件（获取最终结果和文件名）
          else if (line.startsWith("event: metadata")) {
            // 找到metadata的data行
            const metaLine = lines.find(l => l.startsWith("data: ") && "full_text" in JSON.parse(l.slice(6)));
            if (metaLine) {
              const metadata = JSON.parse(metaLine.slice(6));
              const finalAiMsg = {
                id: tempMsgId,
                role: "assistant",
                content: metadata.full_text, // 用后端返回的full_text确保完整性
                docx_file: metadata.filename
              };
              // 最终更新消息列表（替换占位符，添加文件名）
              setMessages(prev => 
                prev.map(msg => msg.id === tempMsgId ? finalAiMsg : msg)
              );
              // 同步预览区为最终完整内容
              setPreviewContent(metadata.full_text);
            }
          }
      ...
      const tempMsgId = Date.now(); // 与后端流式消息占位符ID一致
      if (!response.ok) {
        // 处理非200响应（如401、404）

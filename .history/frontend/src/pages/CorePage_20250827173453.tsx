...
export default function CorePage() {
  ...
  try {
    ...
    // 6. 处理流式响应（重构核心逻辑）
    let currentEvent = "message"; // SSE默认事件类型
    while (true) {
      const { done, value } = await reader.read();
      if (done) break;

      // 解码时添加stream: true，支持多字节字符（如中文）
      const chunkStr = decoder.decode(value, { stream: true });
      const lines = chunkStr.split("\n").filter(line => line.trim() !== "");

      for (const line of lines) {
        // 第一步：识别事件类型（event: xxx）
        if (line.startsWith("event: ")) {
          currentEvent = line.slice(7).trim(); // 更新当前事件类型
          console.log(`[SSE事件] 切换到: ${currentEvent}`);
          continue; // 继续处理下一行的data
        }

        // 第二步：处理对应事件的数据（data: xxx）
        if (line.startsWith("data: ")) {
          const dataStr = line.slice(6).trim();
          if (!dataStr) {
            console.warn("[SSE数据] 空数据片段，跳过");
            continue;
          }
          ...
          try {
            ...
            // 处理元数据事件
              ...
              metadata = data;
              console.log("[SSE元数据] 接收成功:", metadata);
            }

            // 处理错误事件（仅当明确是error事件时）
            else if (currentEvent === "error") {
              console.error("[SSE错误] 后端返回错误:", data);
              throw new Error(data.detail || "流式生成失败");
            }

          } catch (e) {
            // 单个片段解析失败不中断整体流式
            console.warn(`[SSE解析警告] 事件${currentEvent}解析失败:`, e, "数据:", dataStr);
          }
        }
      }
    }

...
export default function CorePage() {
  ...
  const didGenerateTitleRef = useRef(false);

  useEffect(() => {
    if (!currentConv || didGenerateTitleRef.current) return;

    // 找到 AI 消息数量
    const aiMessages = messages.filter(msg => msg.role === "assistant");

    // 当 AI 生成第一条消息后，并且当前对话 title 还是默认值
    if (aiMessages.length === 1 && currentConv.title === "新对话") {
      didGenerateTitleRef.current = true; // 标记已触发，防止重复调用

      api.post(`/api/conversations/${currentConv.id}/generate_title`)
        .then(res => {
          const newTitle = res.data.title || "新对话";
          // 更新全局状态
          dispatch({
            type: "UPDATE_CONVERSATION",
            payload: { id: currentConv.id, title: newTitle }
          });
        })
        .catch(err => {
          console.error("自动生成标题失败", err);
          didGenerateTitleRef.current = false; // 出错可重试
        });
    }

...
const handleApiKeyAdd = async () => {
  try {
    // 假设你已经获取了这些值（可以通过表单输入或默认值）
    const newApiKeyConfig = {
      platform: newConfig.platform,  // 需要从表单或状态中获取
      model_name: newConfig.model,   // 需要从表单或状态中获取
      api_key: newConfig.apiKey,     // 需要从表单或状态中获取
    };

    // 发送请求，包含 API 配置信息
    const response = await api.post("/api/keys", newApiKeyConfig);

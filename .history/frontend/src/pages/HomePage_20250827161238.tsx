...
export default function HomePage() {
  ...
  // 获取历史公文（未修改）
  // 修改 fetchDocuments 函数，强化数组类型校验
const fetchDocuments = async () => {
  setLoadingDocs(true);
  try {
    const res = await api.get("/api/history");
    // 关键：强制校验返回数据是否为数组，兼容不同后端结构
    let documentList = [];
    // 情况1：后端直接返回数组（如 [{}, {}, ...]）
    if (Array.isArray(res.data)) {
      documentList = res.data;
    }
    // 情况2：后端返回包裹在 data 字段中的数组（如 { data: [{}, ...] }）
    else if (res.data?.data && Array.isArray(res.data.data)) {
      documentList = res.data.data;
    }
    // 其他情况（如后端返回错误格式），默认空数组
    else {
      documentList = [];
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      setTemplateTotal(res.data.total); // 接收总条数
    } catch (err) {
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow space-y-4 max-h-[70vh] overflow-auto">
          ...
          {selectedTab === "docs" && (
            <>
              ...
              ) : !Array.isArray(documents) ? ( // 校验是否为数组

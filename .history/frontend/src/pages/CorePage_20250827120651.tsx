...
const handleResizeMove = useCallback((e: MouseEvent) => {
  ...
  // 关键：更新状态后，立即手动更新 ref（不等待 useEffect）
  setMiddleWidth(prev => {
    const updated = Math.max(LAYOUT.middleMinWidth, Math.min(newW, maxW));
    currentWidthRef.current = updated; // 强制同步
    return updated;
  });
  setStartX(e.clientX);

  console.log(`实时宽度: ${currentWidthRef.current}, deltaX: ${deltaX}, 新宽度: ${newW}`);
}, [isResizing, startX]);
    // 3. 拖拽结束：清理事件，重置状态

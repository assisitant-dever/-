...
export default function CorePage() {
  ...
  const handleResizeMove = useCallback((e: MouseEvent) => {
    ...
    // 计算新宽度 + 边界约束
    const maxW = mainW - LAYOUT.rightMinWidth; // 中间栏最大宽度（主宽-右侧最小宽）
    ...
    // 调试信息
    setDebugInfo(`移动：${deltaX}px | 范围：${LAYOUT.middleMinWidth}-${maxW}px | 当前：${newW}px`);
    console.log(`拖拽中：新宽度=${newW}px，主容器宽度=${mainW}px`);
...
useEffect(() => {
  // 定义事件处理函数（避免直接依赖 useCallback 函数，防止引用变化导致问题）
  ...
  // 拖拽中（isResizing 为 true）：绑定全局事件
  if (isResizing) {
    document.addEventListener('mousemove', handleMouseMove);
    document.addEventListener('mouseup', handleMouseUp);
    document.addEventListener('mouseleave', handleMouseLeave);
    console.log("拖拽事件已绑定"); // 调试用，可删除
  }

  // 清理函数：组件卸载或 isResizing 为 false 时，移除事件
  return () => {
    document.removeEventListener('mousemove', handleMouseMove);
    document.removeEventListener('mouseup', handleMouseUp);
    document.removeEventListener('mouseleave', handleMouseLeave);
    console.log("拖拽事件已清理"); // 调试用，可删除
  };
}, [isResizing, handleResizeMove, handleResizeEnd]); // 依赖拖拽核心函数
  // -------------------------- 高度同步（避免错位） --------------------------

...
export default function CorePage() {
  ...
  const handleResizeMove = useCallback((e: MouseEvent) => {
    ...
    const sensitivity = 1;
    const deltaX = (e.clientX - startX) * sensitivity; // 保留原始精度
    ...
    let newW = middleWidth + deltaX; // 不做整数转换
    // 边界约束保留小数
    newW = Math.max(LAYOUT.middleMinWidth, Math.min(newW, maxW));

    setMiddleWidth(newW); // 允许小数宽度
    setStartX(e.clientX);

    console.log(`deltaX: ${deltaX}, newW: ${newW}`); // 查看实际精度

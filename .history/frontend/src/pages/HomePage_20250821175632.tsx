...
export default function HomePage() {
  ...
  const fetchDocuments = async () => {
    try {
      const res = await api.get("/api/history"); // 去掉 /api
  ...
  const fetchConversations = async () => {
    try {
      const res = await api.get("/api/conversations"); // 去掉 /api
  ...
  const fetchTemplates = async () => {
    try {
      const res = await api.get("/api/templates"); // 去掉 /api
  ...
  const deleteTemplate = async (filename: string) => {
    ...
    try {
      await api.delete(`/api/templates/${filename}`); // 去掉 /api
      fetchTemplates();
    } catch (err) { console.error(err); }
  };

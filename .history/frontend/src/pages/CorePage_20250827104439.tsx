...
export default function CorePage() {
  ...
  const updateResizeHandlePosition = () => {
    if (!resizeHandleRef.current || !middlePanelRef.current || !mainContainerRef.current) return;
    
    // 关键修复：获取主容器（中+右栏）的视口偏移，确保位置基准正确
    const mainContainerRect = mainContainerRef.current.getBoundingClientRect();
    // 手柄位置 = 主容器左偏移 + 中间栏宽度 - 手柄一半宽度（确保与中间栏右侧对齐）
    const handleLeft = mainContainerRect.left + middleWidth - 2; // 手柄宽度改为4px，此处减2
  ...
  const handleResizeMove = (e: MouseEvent) => {
    ...
    // 关键修复1：获取主容器的视口信息（左偏移、宽度），作为计算基准
    const mainContainerRect = mainContainerRef.current.getBoundingClientRect();
    // 中间栏新宽度 = 鼠标相对于主容器的X坐标 - 手柄一半宽度（避免超出边界）
    const mouseXInMain = e.clientX - mainContainerRect.left; // 鼠标在主容器内的相对坐标
    let newWidth = mouseXInMain - 2; // 手柄宽度4px，此处减2（居中）
    
    // 关键修复2：处理边界情况（主容器过窄时避免计算错误）
    const minWidth = 400;
    const maxWidth = Math.max(minWidth, mainContainerRect.width - 300); // 确保maxWidth不小于minWidth
    newWidth = Math.max(minWidth, Math.min(newWidth, maxWidth));
    
    setMiddleWidth(newWidth);
    updateResizeHandlePosition(); // 实时同步手柄位置
  };
  ...
  useEffect(() => {
    ...
    // 初始化中间栏宽度（主容器50%，但不超出边界）
    const initMiddleWidth = () => {
      if (!mainContainerRef.current) return;
      const mainContainerRect = mainContainerRef.current.getBoundingClientRect();
      const initWidth = Math.floor(mainContainerRect.width * 0.5);
      const minWidth = 400;
      const maxWidth = Math.max(minWidth, mainContainerRect.width - 300);
      setMiddleWidth(Math.max(minWidth, Math.min(initWidth, maxWidth)));
    };
    initMiddleWidth(); // 单独抽离初始化逻辑，确保ref已挂载
    
    // 同步高度和手柄位置
    const syncLayout = () => {
      syncAllHeights();
      updateResizeHandlePosition();
      initMiddleWidth(); // 窗口变化时重新计算中间栏宽度（关键修复）
    };
    syncLayout();

...
export default function CorePage() {
  ...
  const LAYOUT = {
    ...
    middleMinWidth: 100,      // 中间栏最小宽度（避免挤压）
    rightMinWidth: 100,       // 右侧栏最小宽度（避免消失）
  ...
  const currentWidthRef = useRef<number>(middleWidth);
  ...
  const handleResizeMove = useCallback((e: MouseEvent) => {
    ...
    const maxW = mainW - LAYOUT.rightMinWidth;
    
    let newW = currentWidth + deltaX; // 基于实时宽度计算
    newW = Math.max(LAYOUT.middleMinWidth, Math.min(newW, maxW));

    setMiddleWidth(newW); // 更新状态（仍用于DOM渲染）
    setStartX(e.clientX);

    // 日志：打印实时宽度和计算过程
    console.log(`实时宽度: ${currentWidth}, deltaX: ${deltaX}, 新宽度: ${newW}`);
  }, [isResizing, startX]); // 移除middleWidth依赖（不再使用）
    // 3. 拖拽结束：清理事件，重置状态
  ...
  useEffect(() => {
  currentWidthRef.current = middleWidth;
}, [middleWidth]);
  // -------------------------- 高度同步（避免错位） --------------------------

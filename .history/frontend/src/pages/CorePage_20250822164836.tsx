...
const handleSend = async () => {
  ...
  setInput("");
  setLoading(true);
  if (id === String(currentConv?.id) && messages.length === 0) {
  ...
  try {
    ...
    // ✅ 如果有选中的模板，作为格式参考
    if (selectedTemplate) {
      formData.append("template_content", selectedTemplate);
    }
    ...
    setPreviewURL(`/api/download/${filename}`); // 用于 iframe 预览
